--[=[
	...
	Arguments:
		ACCENT: accent pattern, a number (1-6, 4*, 6*) or a Zaliznyak-style
		   value (a-f, b', d', f', f''), or multiple values separated by
		   commas; if omitted, defaults to 1 or 2 depending on the position
		   of stress on the lemma
		LEMMA: lemma form (i.e. nom sg or nom pl); or the stem, if the ending
		   is included in the declension field; required in the first stem
		   set (i.e. first set of arguments separated by "or"), can be
		   omitted in later stem sets to default to lemma of previous stem set
	...
	Declension field:
		...
		GENDER if present is m, f, or n; for regular nouns, required if the
			lemma ends in -ь or is plural, ignored otherwise.
		...
		DECLTYPE is an explicit declension type, usually the same as the
			...
			ье or ьё, ь-m, ь-f, -а, ь-я, о-и or о-ы, ин, ёнок or онок.
...
12. Dereducing masculine plural. [IMPLEMENTED; NEED TO TEST.]
...
function export.do_generate_forms(args, old)
	...
	-- Generate forms for a given stem set, stem and declension class.
	-- Extracted into a function to avoid indenting a lot of code and
	-- make merging easier.
	local function do_stem_set_decl(stem_set, stem, decl_class)
	...
	for _, stem_set in ipairs(stem_sets) do
		local decl_classes = stem_set[3] or ""
		if decl_classes == "manual" then
			decl_classes = "$"
			args.manual = true
			if #stem_sets > 1 then
		...
		default_stem = stem
		decl_classes = rsplit(decl_classes, ",")
		if #decl_classes > 1 then
			track("multiple-declensions")
			insert_cat("~ with multiple declensions")
		end
		for _, decl_class in ipairs(decl_classes) do
			do_stem_set_decl(stem_set, stem, decl_class)
		end
	end
